...
class Matrix{
  ...
  public:
	...
	Matrix<T>& operator= (initializer_list<initializer_list<T>>);
	void swap(Matrix<T>&);
	Matrix<T>& E(); // 置为单位矩阵
...
// for test
	...
	Matrix<T> inverse(double det=0) const; // 行列式逆矩阵
...
__t(T)
Matrix<T>&
Matrix<T>::operator= (initializer_list<initializer_list<T>> il)
{
	delete[] bp;
	return *new(this) Matrix<T>(il);
}
...
__t(T)
Matrix<T>
Matrix<T>::inverse(double det) const
{
	if ( ! det )
		det = value();
	if ( ! det )
